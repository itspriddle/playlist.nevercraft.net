    ...
    this.each(function() {
      ...
      $.getJSON(lastUrl, function(data){
        $.each(data.recenttracks.track, function(i, item){
          ...
          var url = stripslashes(item.url);
          var song = item.name;
          var artist = item.artist['#text'];
          var album = item.album['#text'];

          var nowplaying = item['@attr'] && item['@attr']['nowplaying'] || false;

          if (nowplaying) {
            var played_at = 'Now Playing <span class="nowplaying"><image src="images/icon_eq.gif" /></span>'
          } else {
            // if (i < 3) alert(item.date.uts);
            var time = item.date.uts * 1000;
            var then = new Date(time);
            var played_at = $.relatizeDate.timeAgoInWords(then)
          }
          ...
          $current.find('[class=lfm_played_at]').html(played_at);
